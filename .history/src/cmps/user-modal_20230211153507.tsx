...
export function UserModal({ onLogout, toggleUserModal }: UserModalProps) {
    ...
    useEffect(() => {
        if (modalOptions.dragPercent > 70) toggleUserModal(undefined!)
    }, [modalOptions])

    function onDown(ev: MouseEvent | TouchEvent<HTMLDivElement>) { 
        setIsDragging(true)
        const mousePosY = getClientY(ev)
        const { top, height } = modalRef.current!.getBoundingClientRect()
        const dragPercent = getDragPercent(undefined, height)
        console.log('dragPercent', dragPercent)
        console.log('diff', modalOptions.diff)
        console.log('height', height)
        setModalOptions((prevOpts: IModalOpts) => ({ ...prevOpts as IModalOpts, modalTop: top, modalHeight: height, dragPercent, dragStartY: mousePosY}))
    }
